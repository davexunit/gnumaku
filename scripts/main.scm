...
(define (bullet-stuff system bullet-list)
  (coroutine
   (for-each
    (lambda (bullet)
...
(define (cool-reverse system bullet-list)
  (coroutine
   (for-each
    (lambda (bullet)
      (set-bullet-sprite! bullets bullet 2)
...
(define (splosion-bullet system bullet-list)
  (coroutine
   (for-each
    (lambda (bullet)
      (set-bullet-sprite! bullets bullet 2)
      (set-bullet-speed! bullets bullet 80))
    bullet-list)
   (wait 1)
   (for-each
    (lambda (bullet)
	...
	(let ((new-bullet (make-bullet system)))
	  (when (< i 8)
	    (set-bullet-sprite! system new-bullet 0)
	    (set-bullet-speed! system new-bullet 120)
	    (set-bullet-position! system new-bullet (bullet-x system bullet) (bullet-y system bullet))
	    (set-bullet-direction! system new-bullet (random 360))
	    (set-bullet-angular-velocity! system new-bullet (random 10))
	    (set-bullet-acceleration! system new-bullet (random 40))
	    (loop (1+ i)))))
      (kill-bullet system bullet))
    bullet-list)))

(define (large-bullet system bullet-list)
  (for-each 
   (lambda (bullet)
     (set-bullet-sprite! system bullet 2)
     (set-bullet-speed! system bullet 100))
   bullet-list))

(define (diamond-bullet system bullet-list)
  (for-each 
   (lambda (bullet)
